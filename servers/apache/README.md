...
- `MesiCacheKeyTemplate "mesi:$${url}:${header:Accept-Language}:${cookie:segment}"` — Cache key template evaluated by libgomesi's `mesi.BuildCacheKey`. Placeholders: `${url}` (verbatim include URL), `${header:Name}` (case-insensitive header lookup), `${cookie:Name}` (case-insensitive cookie lookup). Unknown placeholders stay literal; NULL/empty falls back to URL-only `DefaultCacheKey` (`"mesi:" + url`) for backward compat. Without a cache backend (`MesiCacheBackend` empty/unset) the template has no effect — there is no cache to key into. Requires `libgomesi.so` with `ParseWithConfigCtx` (older builds silently use URL-only keys and log `MesiCacheKeyTemplate set but libgomesi lacks ParseWithConfigCtx`). At most 4096 bytes; control chars/DEL rejected. **Apache interpolation caveat:** Apache's `ap_resolve_env` replaces every undefined `${VAR}` with "" before the module sees it (`AH00111: Config variable ${url} is not defined`, `AH00111: Config variable ${header:...}` etc are _not_ interpolated because `:` is not a valid env-var character, but `${url}` _is_). To get a literal `${url}` in `httpd.conf`, escape the dollar: `MesiCacheKeyTemplate "mesi:$${url}:${header:Accept-Language}"` (Apache delivers `$$` verbatim — `ap_resolve_env` still warns `AH00111` and keeps the extra `$` — the module normalizes `$${`→`${` at config load, so the effective template is `mesi:${url}:...` at any position/repetition like `"mesi:${header:X}:$${url}:${cookie:C}"` or a lone `"$${url}"`). A lone `"${url}"` or any template where `${url}` was mangled would contain `::` (e.g. `"mesi::${header:X}"`) or end in `:` (e.g. `"mesi:${header:X}:"`, `"${url}"→""`), both rejected at config load with `MesiCacheKeyTemplate: Apache config interpolation replaced ${url} (AH00111); escape the dollar sign as $${url} in httpd.conf` — no silent wrong keys. **Limitation:** legit templates intentionally containing `::` (e.g. `mesi:${url}::section`) or intentionally ending in `:` (e.g. `mesi:${url}:`) are now unrepresentable in Apache configs because `::`/trailing `:` are the mangled-template signal — fail-loud per project rules, but document the tradeoff. To keep `::` literally, insert a separator (`-`, `_`) or move the literal out of the template. All other platforms (Caddy, RoadRunner, Traefik, php-ext, CLI) take the template verbatim — Apache is the only one that interpolates, hence the `$$` requirement. Copy-pasteable example: `MesiCacheKeyTemplate "mesi:$${url}:${header:Accept-Language}"` (single include), `"mesi:$${url}:${header:Accept-Language}:${cookie:segment}"` (header+cookie), `"mesi:${header:X}:$${url}:${cookie:C}"` (mid-position — also requires `$$`).

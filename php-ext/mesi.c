...
/* Per-parse concurrent include-fetch cap (#206). Matches libgomesi's
 * config.MaxMaxConcurrentRequests (999999999 — the #170 transport-derived
 * bound: the value crosses as a JSON number and out to Apache as a C
 * `int`, guarded at 9 digits by parse_nonneg_int; the Go const lives in
 * libgomesi/internal (unimportable from C — keep in sync manually)).
 * 0 is the documented "unlimited" value (the core only installs the
 * admission-control semaphore when the value is > 0, mesi/parser.go);
 * the ABSENT key likewise means unlimited — the value every positional
 ...
 *                            runs — never a crash.
 *   max_concurrent_requests: optional integer. Caps the number of
 *                            CONCURRENT <esi:include> HTTP fetches within
 *                            ONE parse_with_config() call — the
 *                            admission-control semaphore the core installs
 *                            when the value is > 0 (mesi/parser.go), so
 *                            per call, NOT global across PHP calls or
 *                            requests (each call builds its own
 *                            semaphore). Range [0, 999999999] (mirrors
 *                            libgomesi's config.MaxMaxConcurrentRequests
 *                            — the #170 transport-derived cap, reflected
 *                            in MESI_MAX_MAX_CONCURRENT_REQUESTS above).
 *                            Absent => 0 = UNLIMITED — the value every
 *                            positional Parse* path leaves (byte-identical
 *                            to previous behaviour). Explicit 0 is the
 *                            documented "unlimited" value: with a current
 *                            libgomesi.so there is NO observable
 *                            difference from an absent key (both resolve
 *                            to 0 Go-side; the only difference is the
 *                            routing — an explicit key routes through
 *                            ParseJson, so against an old libgomesi.so
 *                            without that symbol only the explicit key
 *                            warns, same as timeout/max_response_size).
 *                            Negatives are rejected instead of silently
 *                            behaving like 0 (the core only warns and
 *                            normalizes them to 0 — #329). Malformed
 *                            explicit values (string "10", float 1.5,
 *                            bool, null, array) are rejected with
 *                            E_WARNING and the function returns false —
 *                            same strict contract as
 *                            timeout/max_response_size/cache_ttl. When
 *                            the key is present the call is routed
 *                            through libgomesi's ParseJson entry point
 *                            (#167) as {"maxConcurrentRequests":N} (the
 *                            #170 schema key) together with every other
...
PHP_FUNCTION(parse_with_config) {
    ...
    /* max_concurrent_requests: per-parse admission cap on CONCURRENT
     * include fetches, [0, MESI_MAX_MAX_CONCURRENT_REQUESTS]. Absent => 0
     * = unlimited — the value the positional path always left
     * (byte-identical to previous behaviour). max_concurrent_requests_set
     * distinguishes "explicit key" from "absent" so the blob only renders
     * the key when set — and like timeout/max_response_size, only an
     * explicit value routes the call through ParseJson (#206; Apache
     * #170 pattern). */
    ...
    if (config != NULL && Z_TYPE_P(config) == IS_ARRAY) {
        ...
        /* max_concurrent_requests: per-parse concurrent include-fetch cap,
         * range [0, MESI_MAX_MAX_CONCURRENT_REQUESTS]. Strict validation:
         * an absent key keeps the documented default (0 = unlimited,
         * byte-identical to previous behaviour), but an explicit
         * malformed or out-of-range value is NEVER silently coerced —
         * non-integers (string "10", float 1.5, bool, null, array) and
         * values outside the range (-1, -100, 1000000000, PHP_INT_MAX)
         * emit E_WARNING naming the option and the call returns false,
         * same contract as timeout/max_response_size/cache_ttl.
         * 0 is deliberately ACCEPTED as the documented "unlimited" value
         * (the core only installs the admission semaphore when
         * MaxConcurrentRequests > 0 — mesi/parser.go); negatives are
         * rejected because the core only warns and normalizes them to 0
         * (#329), i.e. a malformed explicit value would silently pass as
         * the documented "unlimited". */
        val = zend_hash_str_find(Z_ARRVAL_P(config), "max_concurrent_requests",
                                 sizeof("max_concurrent_requests") - 1);
        if (val != NULL) {
            if (Z_TYPE_P(val) != IS_LONG) {
                php_error_docref(NULL, E_WARNING,

--TEST--
parse_with_config() max_workers: option parsing and range validation
--SKIPIF--
<?php if (!extension_loaded('mesi')) die('skip'); ?>
--FILE--
<?php
// Strict validation contract (#211): an absent key keeps the documented
// default 0 = the library runtime.NumCPU()*4 (byte-identical to previous
// behaviour — every positional Parse* path leaves EsiParserConfig.MaxWorkers
// at its zero value), but every malformed EXPLICIT value must produce
// E_WARNING naming the option and make parse_with_config() return false —
// never a silent coercion. The rejection matters MORE here than for
// max_concurrent_requests: the core substitutes NumCPU*4 for any value
// <= 0 SILENTLY with NO warning (#456 — unlike #329's warn+normalize), so
// this PHP-side check is the only protection a PHP caller gets.
// Accepted values route the call through libgomesi's ParseJson entry point;
// a warning here would also mean the symbol lookup failed, so warnings=0
// doubles as proof the ParseJson path is live.
$warnings = [];
...
// --- accepted boundary classes: 0 (library default), 1 (min cap), 8 (the
//     issue's typical value), 999999999 (the documented cap, #171) ---
list($r,$w)=run(['max_workers'=>0]);
echo "zero_result=".($r===false?'false':'string')." zero_warnings=".count($w)."\n";
list($r,$w)=run(['max_workers'=>1]);
echo "min1_result=".($r===false?'false':'string')." min1_warnings=".count($w)."\n";
list($r,$w)=run(['max_workers'=>8]);
echo "typ8_result=".($r===false?'false':'string')." typ8_warnings=".count($w)." typ8_body=".($r===false?'':$r)."\n";
list($r,$w)=run(['max_workers'=>999999999]);
echo "cap_result=".($r===false?'false':'string')." cap_warnings=".count($w)."\n";

// --- absent key: documented default 0 = library default, positional path,
//     no warnings ---
list($r,$w)=run([]);
...
list($r,$w)=run(['max_workers'=>-1]);
echo "warn_names_option=".(count($w)===1 && strpos($w[0], 'max_workers') !== false ? 'yes':'no')."\n";
?>
--EXPECT--
zero_result=string zero_warnings=0
min1_result=string min1_warnings=0
typ8_result=string typ8_warnings=0 typ8_body=plain-ok

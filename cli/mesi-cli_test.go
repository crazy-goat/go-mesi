...
func TestValidateMaxWorkers(t *testing.T) {
	// Boundary classes per project rules: accepted 0 / 1 / typical / max,
	// rejected negative / large negative / max+1 (1000000000 — the #171
	// transport-derived 9-digit cap, C int / parse_nonneg_int). 0 is the
	// documented "library default" (the core substitutes NumCPU*4 for
	// any value <= 0, mesi/parser.go:119-122), not a sentinel.
	rangeText := fmt.Sprintf("[0, %d]", maxMaxWorkers)
	tests := []struct {
		name    string
		value   int
		wantErr bool
	}{
		{"zero (documented library default) accepted", 0, false},
		{"one accepted", 1, false},
		{"typical 5 accepted", 5, false},
		{"accepted max (999999999)", maxMaxWorkers, false},
		{"rejected negative", -1, true},
		{"rejected large negative", -1000000, true},
		{"rejected max+1 (1000000000)", maxMaxWorkers + 1, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateMaxWorkers(tt.value)
			if !tt.wantErr {
				if err != nil {
					t.Fatalf("validateMaxWorkers(%d) = %v, want nil", tt.value, err)
				}
				return
			}
			if err == nil {
				t.Fatalf("validateMaxWorkers(%d) = nil, want error", tt.value)
			}
			// The error must name the flag and the valid range.
...
func TestCLI_maxWorkersFlagInHelp(t *testing.T) {
	stdout, stderr, _ := runCLI(t, "-h")
	output := stdout + stderr
	if !strings.Contains(output, "-max-workers") {
		t.Errorf("expected -max-workers in help output, got stdout=%q stderr=%q", stdout, stderr)
	}
	if !strings.Contains(output, "0 = NumCPU*4") {
		t.Errorf("expected '0 = NumCPU*4' in help output, got stdout=%q stderr=%q", stdout, stderr)
	}
	// Pin the default: the flag package only prints "(default …)" for
	// non-zero defaults, so the ABSENCE of "(default" on this flag's usage
	// line proves the default is 0 (= the constructor-derived library
	// default path, byte-identical to the pre-flag behaviour) — same pin
	// as -max-concurrent-requests, opposite of -max-response-size's
	// "(default 10485760)".
	lines := strings.Split(output, "\n")
	for i, line := range lines {
		if !strings.Contains(line, "-max-workers") {
			continue
		}
		if i+1 >= len(lines) {
			t.Fatalf("no usage line after %q in help output", line)
	...
	t.Errorf("-max-workers not found line-wise in help output")
}

func TestCLI_maxWorkersFlagValidation(t *testing.T) {
	tmpDir := t.TempDir()
	inputFile := filepath.Join(tmpDir, "input.html")
	if err := os.WriteFile(inputFile, []byte("<!--esi Hello-->"), 0644); err != nil {
		t.Fatal(err)
	...
	tests := []struct {
		...
		{name: "accepted zero (library default NumCPU*4)", arg: "-max-workers=0", wantExit: 0},
		{name: "accepted one", arg: "-max-workers=1", wantExit: 0},
		{name: "accepted typical", arg: "-max-workers=5", wantExit: 0},
		{name: "accepted max (999999999)", arg: "-max-workers=999999999", wantExit: 0},
		{
			// The core would silently substitute NumCPU*4 for this with
			// NO warning (mesi/parser.go:119-122, #456) — the CLI fails
			// loud instead (deviation from the issue's AC
			// "negative → default", per the #415/#186/#192 precedent).
			name: "rejected negative", arg: "-max-workers=-1", wantExit: 1,
			wantErrs: []string{"max-workers", "999999999", "-1"},
		},
		{
			name: "rejected large negative", arg: "-max-workers=-1000000", wantExit: 1,
			wantErrs: []string{"max-workers", "999999999", "-1000000"},
		},
		{
			// 1000000000 == maxMaxWorkers+1: the #171 transport-derived
			// 9-digit cap.
			name: "rejected max+1 (1000000000)", arg: "-max-workers=1000000000", wantExit: 1,
			wantErrs: []string{"max-workers", "999999999"},
		},
		{
			name: "rejected above int64 (flag pkg parse error)", arg: "-max-workers=9223372036854775808", wantExit: 2,
			wantErrs: []string{"max-workers"},
		},
		{
			name: "rejected non-integer (flag pkg parse error)", arg: "-max-workers=abc", wantExit: 2,
			wantErrs: []string{"max-workers"},
		},
	}
	for _, tt := range tests {
